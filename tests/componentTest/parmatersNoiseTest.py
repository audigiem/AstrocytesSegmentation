...
class EventDetectionTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_dirs = [
            "/home/matteo/Bureau/INRIA/codePython/outputdir/testDir/",
            "/home/matteo/Bureau/INRIA/codePython/outputdir/checkDir20/",
        ]
        self.data = [
            load_data(dir_path + "dynamic_image_dF.tif") for dir_path in self.test_dirs
        ]
    ...
    def test_noise_estimation(self):
        ...
        for data in self.data:
            ...
            std_optimized = estimate_std_over_time_optimized(
                data, self.xmins, self.xmaxs
            )
            # assert that the standard deviations are close enough
            self.assertTrue(
                np.allclose(std, std_optimized, rtol=1e-5, atol=1e-8),
