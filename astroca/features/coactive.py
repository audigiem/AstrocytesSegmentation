...
def compute_coactive_csv(features: dict, path_output_dir: str, params_values: dict = None) -> None:
    """
...
def _calculate_coactive_statistics(events_data: List[Dict], voxel_size_x: float,
                                   voxel_size_y: float, voxel_size_z: float) -> Dict:
    """
...
def write_csv_coactive(path_output_dir: str, list_list_label: List[List[int]],
                       list_t0: List[int], list_coactive_stats: List[Dict]) -> None:
    """
        ...
        with open(output_path, 'w', newline='', encoding='utf-8') as file:
            ...
            for t0, labels, stats in zip(list_t0, list_list_label, list_coactive_stats):
                ...
                if len(labels) > 1:
                    ...
                    distances_str = "[" + '; '.join(f"{d:.3f}" for d in stats['all_distances']) + "]"
                else:
                    labels_str = labels
                    distances_str = "None"
                csv_writer.writerow([
...
def analyze_coactive_patterns(list_coactive_stats: List[Dict]) -> Dict:
    ...
    multi_event_stats = [stats for stats in list_coactive_stats if stats['event_count'] > 1]
    ...
    return {
        'total_coactive_groups': len(list_coactive_stats),
        'single_event_groups': len([s for s in list_coactive_stats if s['event_count'] == 1]),
        ...
        'compact_groups': len([s for s in multi_event_stats if s['mean_distance'] < 10.0]),  # < 10µm
        'dispersed_groups': len([s for s in multi_event_stats if s['mean_distance'] >= 10.0])  # >= 10µm
    }


def get_coactive_summary(features: dict, path_output_dir: str, params_values: dict = None) -> Dict:
    """
    ...
    for t0 in unique_t0_values:
        t0_events = [(eid, ed) for eid, ed in localized_events if ed['T0 [frame]'] == t0]
